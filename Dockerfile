...
RUN tmpdir=$(mktemp -d) \
    ...
    && find "$tmpdir" \( -type f -o -type l \) -name "libwasmedge*.so*" -exec cp -a {} /opt/wasmedge/lib/ \; \
    && rm -rf "$tmpdir" \
...
RUN curl -sL \
    "https://huggingface.co/second-state/All-MiniLM-L6-v2-Embedding-GGUF/resolve/main/ggml-model-f16.gguf" \
