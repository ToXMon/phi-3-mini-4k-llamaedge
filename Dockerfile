...
#
# FIX: find WITHOUT -type f finds symlinks too.
# cp -a preserves symlinks when copying.
# The .so chain is: libwasmedge.so -> libwasmedge.so.0 -> libwasmedge.so.0.0.0
# All three must be present for the runtime linker to work.
...
# ── WasmEdge runtime (find without -type f + cp -a preserves symlinks) ──
RUN tmpdir=$(mktemp -d) \
    ...
    && mkdir -p /opt/wasmedge/bin /opt/wasmedge/lib \
    && find "$tmpdir" -name "wasmedge" ! -path "*/lib/*" -exec cp -a {} /opt/wasmedge/bin/ \; \
    && find "$tmpdir" -name "libwasmedge*" -exec cp -a {} /opt/wasmedge/lib/ \; \
    && rm -rf "$tmpdir" \
    && test -f /opt/wasmedge/bin/wasmedge \
    || { echo "FATAL: wasmedge binary not found"; exit 1; }

# ── wasi_nn-ggml plugin (find without -type f + cp -a preserves symlinks) ──
RUN tmpdir=$(mktemp -d) \
